    ...
    public MischiefCommand(MischiefPlugin plugin) {
        ...
        executor = new Command(getLabel()) {
            ...
            public @NotNull List<String> tabComplete(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
                ...
                if(cmd != null)
                    parts = new ArrayList<>(cmd.executor.tabComplete(sender, cmd.getLabel(), removeFirst(args)));
                if(parts == null && hasMainPermission(sender)) {
                    ...
                    parts = new ArrayList<>(onTab(sender, plugin, args));
                    for(String key : subCommands.keySet()) {
            ...
            public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
                ...
                MischiefCommand cmd = getSubCommand(args);
                if(cmd != null) {
                    cmd.executor.execute(sender, cmd.getLabel(), removeFirst(args));
                    return true;
                }

                if(hasMainPermission(sender)) {
                    switch (onCommand(sender, this, plugin, label, args)) {
